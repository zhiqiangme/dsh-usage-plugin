...
dsh-usage-plugin 是 DeepSeek Harness 生态的**用量与消耗统计插件**（DSH plugin，Host + Client 双面一体包）。装好后在 WebUI 顶部「对话」「轨迹」之后会出现 **「用量」** 与 **「余额」** 两个 tab：
...
- **余额**：用当前配置的 `DEEPSEEK_API_KEY` 查询 DeepSeek 账户余额；并支持 **百炼 Token Plan 配额查询**（复用 `bl auth login --console` 的控制台 OAuth token，无需阿里云 AccessKey，展示本周配额已用百分比与重置时间）。
- **导出**：CSV / JSON / **PNG 长图**（按最新在前展示，最多含最近 2000 条，超出会提示；PNG 报告含高峰 / 空闲消耗分列统计），可导出到任意目录（原生目录选择器），导出后自动打开所在目录。
- **导入**：选择文件（JSON / CSV）合并导入，按时间去重。
- **历史回填（事后扫描）**：插件自 v1.15.0 起除了「实时探针」（监听 `llm/stream` 记录每一次模型调用）之外，还支持**事后扫描**——通过 Harness 的 `ctx.sessionPersistence` 服务读取本地已落盘的会话事件日志，把**插件激活之前**发生的调用补进同一份记录。面板顶部「扫描历史 / 扫描全部工作区 / 深扫」三个按钮可手动触发，插件加载后也会自动对当前工作区做一次增量扫描。扫描记录带「日志」徽标，与探针记录同表展示。
  - 默认**只扫当前工作区**，且**跳过实时探针已经记录过的会话**，因此不会与探针双计；重扫同一段日志按 `(sessionId, turn, step)` 原地替换，天然幂等（无需持久化折叠状态）。
  - 增量游标（`.dsh-usage/dsh-usage/scan-cache.json`）与不透明 `revision` 结合：日志未变化时一次读取都不发；日志被截断/重写时自动从 seq 0 重扫。
  - 已知局限：被中断的调用（aborted / error / timeout）**没有** usage 事件落到日志里，扫描无法回填其 token，因此这类历史调用的消耗仍按 0 计（与「本地统计与官方后台的差异」横幅说明一致）；扫描拿不到 `purpose` 与 `finishReason`（这两个字段只存在于实时流中）。
...
### 用量
![用量](./docs/assets/usage-overview.png)

### 余额
![余额](./docs/assets/balance-query.png)
...
- 刷新 http://127.0.0.1:3080 ，顶部「对话」「轨迹」之后会出现 **「用量」** 和 **「余额」** 两个 tab；设置里也有对应入口。
- 「用量」面板内含 **概览 / 用量日历 / 缓存命中列表 / 价格表** 四个子页签。
- 发一条消息后，「用量」面板应出现本次调用的 token / 消耗记录。

### 5. 配置（余额查询需要）

「余额」使用当前配置的 `DEEPSEEK_API_KEY`：在 **设置 → 模型** 中配置 API Key（与跑对话用的同一个 key），然后打开「余额」tab 点「查询余额」。

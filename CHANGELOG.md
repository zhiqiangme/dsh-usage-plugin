...
## v1.16.5-local.22（本地构建 / Local build）

### 修复 / Fixes

- 持久化使用同目录临时文件刷盘后原子替换；初始化遇到损坏 JSON、错误结构或读取失败时保留原件，停止初始化并报告错误，不再覆盖为空数组或悄悄切换数据目录。
- 保留扫描来源、事件序号、轮次、步骤和结算槽位。新实时调用带独立 UUID；旧记录、导入与迁移使用复合身份去重，同毫秒不同请求不再互相覆盖。
- 扫描按宿主结算槽位折叠，重试独立累计；增量缓存保存折叠状态，深扫强制完整读取。用量成功落盘后才提交扫描游标。
- 深扫先折叠重复结算，仅吸收会话、服务商、模型、用量及时间匹配的唯一普通探针。标题等辅助调用保持独立，多个候选报告 ambiguous 并保留探针，不再追加副本。已丢失身份的旧数据不能凭时间猜测恢复。
- 取消仅凭数值相等清零 cacheWrite 的迁移逻辑，真实缓存写入原样保留。历史已清零的数值不会凭空恢复。
- 面板恢复唯一匹配记录的历史汇率、日期和计费起点；缺少历史汇率的 DigitalOcean 面板费用显示“—”，不使用当前汇率伪算。
- 验证：110 项测试通过；74 个本机会话的扫描四桶累计与宿主一致，模拟落盘重载后重复深扫的记录数和用量保持不变。验证未重写用户会话日志或用量存储。

...
import { readConversationUsage } from './conversation-usage.js'
    ...
    try {
      ...
      function costFor(rec, regime) {
        ...
        if (provider === 'digital-ocean' || provider === 'digitalocean') {
          ...
          if (!p) return 0
          const rate = Number(rec.usdCnyRate || (rec.origin === 'scan' ? 0 : FX.rate) || 0)
          if (!(rate > 0)) return 0
        ...
        if (regime === 'auto') {
          if ((rec.pricingTime || rec.time) < EFFECTIVE_AT) {
            const p = PRICING.base[mk]
            if (!p) return 0
            return (hit * p.cacheHit + miss * p.cacheMiss + out * p.output) / 1e6
          }
          ...
          if (!pv) return 0
          const p = isPeak(rec.pricingTime || rec.time) ? pv.peak : pv.offPeak
          return (hit * p.cacheHit + miss * p.cacheMiss + out * p.output) / 1e6
        }
        const pv = PRICING.peakValley[mk]
        if (!pv) return 0
      ...
      function projectRecord(r) {
        return {
          ...
          costUnavailable: r.origin === 'scan' && /^(digital-ocean|digitalocean)$/i.test(r.provider) && !(r.usdCnyRate > 0),
          ...
          peak: isPeak(r.pricingTime || r.time)
        }
      }
      ...
      function computeAgg(items) {
        ...
        for (const it of items) {
          if (it.costUnavailable) agg.costUnavailable = true
      ...
      function persistNow(strict = false) {
        if (!dataPath || !persistOk) return Promise.resolve()
        const text = JSON.stringify(records)
        const pending = writeChain.then(() => nfsWriteText(dataPath, text)).then(() => { persistError = ''; notifyChanged() })
        writeChain = pending.catch((e) => { persistError = msg(e); push('persist-failed: ' + msg(e)) })
        return strict ? pending : writeChain
      }
      ...
      async function runHistoryScan(options = {}) {
        ...
        try {
          await ensureInit()
          if (!persistOk) throw new Error(persistError || '用量存储不可用')
          await loadScanCache()
          const nextCache = structuredClone(scanState.cache)
          ...
          commitScanRevisions(nextCache, result.revisions)
          // 丢弃已不存在的会话，避免游标缓存无限增长。
          pruneScanCache(nextCache, new Set(sessions.map((snapshot) => String((snapshot && snapshot.header && snapshot.header.id) || '')).filter(Boolean)))
          // 用量先落盘，成功后才发布游标；写入失败下次仍可重试。
          await persistNow(true)
          scanState.cache = nextCache
          await saveScanCache()
          scanState.last = {
            at: startedAt,
            finishedAt: Date.now(),
        ...
        try {
          {
            ...
            if (Array.isArray(arr) && arr.length > 0) {
              ...
              for (let i = 0; i < arr.length; i++) {
                ...
                if (!rec || existing[recordKey(rec)]) continue
                existing[recordKey(rec)] = true
                records.push(rec)
              }
              if (records.length > MAX_RECORDS) records.splice(0, records.length - MAX_RECORDS)
          ...
          // 数据损坏时禁止试写和切换目录，保留原件并明确报错。
          return { ok: false, fatal: true, error: '用量文件读取失败，原件已保留：' + msg(e) }
        }
        try {
      ...
      async function initPersistence() {
        ...
        for (const c of candidates) {
          ...
          if (r.fatal) { lastError = r.error; break }
        ...
        ctx.on('llm/stream', function (options, next) {
          ...
          async function* observe() {
            try {
              ...
              if (usage) {
                // cacheWriteTokens 只存上游真实上报值：DeepSeek 系 provider 不上报该字段（记 0）。
                // 旧版在此用 inputTokens 兜底，等于伪造一笔与未命中相等的缓存写入，
                // 四桶合计（input+cacheRead+cacheWrite+output）会把未命中双计，
                // 使「总 token」比宿主状态栏恰好大出累计未命中数。「缓存写入」列的
                // 展示兜底由客户端负责（lib/client.js 写入列），不再污染记录。
      ...
      async function routeApi(body) {
        ...
        switch (action) {
          ...
          case 'tokenForMessage': {
            ...
            // 对话面板以宿主日志为准：补齐漏记，排除辅助请求，保留重试用量。
            // 不套用 isRealCall 过滤，以免丢掉宿主已计入的未知模型或内部结算。
            let sessionRecs
            try {
              sessionRecs = enrichBillingMetadata(await readConversationUsage(ctx.get('sessionPersistence'), sid), records)
            } catch (error) {
              return fail('无法读取会话 Token 数据：' + msg(error))
            }
          ...
          case 'import': {
            ...
            for (const raw of parsed) {
              ...
              if (existing[recordKey(rec)]) { skipped++; continue }
              existing[recordKey(rec)] = true
              records.push(rec)
              imported++
            }

...
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      // 已知从不上报 cacheWriteTokens 的 provider（DeepSeek 系 API 无缓存创建字段）。
      // 这些 provider 的历史记录里 cacheWriteTokens === inputTokens 的，都是旧版
      // 兜底伪造值（见探针注释），入库时统一清零，否则四桶合计继续双计未命中。
      const NO_CACHE_WRITE_PROVIDERS = ['deepseek', 'deepseek-official', 'siliconflow', 'digital-ocean', 'digitalocean']
      ...
      function normalizeRecord(raw) {
        ...
        if (rec.inputTokens > 0 && rec.cacheWriteTokens === rec.inputTokens
          && NO_CACHE_WRITE_PROVIDERS.includes(rec.provider.trim().toLowerCase())) {

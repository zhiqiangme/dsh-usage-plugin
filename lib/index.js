...
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      // 已知真实上报 cacheWriteTokens 的 provider（当前为空：实测 dsh 生态全部
      // provider——deepseek-official 与 buddy / workbuddy / codearts 等 harness
      // 路由——都不上报缓存创建字段，重启后探针记录 write 全为 0）。
      // 反转判定：除本名单外，历史记录中 cacheWriteTokens === inputTokens 且
      // miss > 0 的一律视为旧版兜底伪造值，入库时清零——天然覆盖任意路由名；
      // 未来真出现上报者时往这里加名字即可。
      const KNOWN_CACHE_WRITE_PROVIDERS = []
      ...
      function normalizeRecord(raw) {
        ...
        if (rec.inputTokens > 0 && rec.cacheWriteTokens === rec.inputTokens
          && !KNOWN_CACHE_WRITE_PROVIDERS.includes(rec.provider.trim().toLowerCase())) {
      ...
      async function routeApi(body) {
        ...
        switch (action) {
          ...
          case 'tokenForMessage': {
            ...
            let sessionRecs
            try {
              sessionRecs = await readConversationUsage(ctx.get('sessionPersistence'), sid)
            } catch (error) {

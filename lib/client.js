...
var api = function (payload) {
    ...
    function SidebarUsageEntry(props) {
      ...
      var state = React.useState({ provider: null, amount: null, note: "" });
      var data = state[0], setData = state[1];
      ...
      React.useEffect(function () {
        var alive = true;
        function load() {
          probeFirstBalance(SIDEBAR_BALANCE_ORDER).then(function (hit) {
            if (alive && hit) setData(hit);
          }).catch(function () {});
        }
        load();
      ...
      function onClick() {
        try {
          if (typeof props !== "undefined" && props && typeof props.openBalanceView === "function") {
            props.openBalanceView();
          }
        } catch (e) {}
      }

      var hasAmount = data.amount != null;
      var label = data.provider ? data.provider.name : t("余额");
      var title = hasAmount
        ? label + " · " + data.amount + (data.note ? " · " + data.note : "")
        : t("余额") + t("（未配置查询凭据）");

      return el("div", { className: wide ? "dsh-usage-side" : "dsh-usage-side rail" },
        el("button", {
          type: "button", className: "dsh-usage-sideBtn", onClick: onClick,
    ...
    /**
     * 依次探测服务商，返回第一个"已配置凭据且成功查到余额"的结果。
     * 串行而非并行：避免一次打开就并发打多个上游接口，且顺序即优先级。
    ...
    function probeFirstBalance(order) {
      ...
      function next() {
        ...
        var id = list[i++];
        // 先问凭据状态，未配置就跳过（不发上游请求）。
        return api({ action: "balanceCredentialStatus", provider: id }).then(function (st) {
          if (!st || !st.ok || !st.configured) return next();

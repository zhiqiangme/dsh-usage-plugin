...
var api = function (payload) {
    ...
    /**
     * 侧边栏底部入口：显示本月已消耗，点击切到「用量」页签。
     * props: { wide（侧边栏是否展开）, t（本插件命名空间不可用，忽略） }
     */
    function SidebarUsageEntry(props) {
      var wide = !props || props.wide !== false;
      var state = React.useState({ cost: null, calls: 0 });
      var data = state[0], setData = state[1];
      ...
      React.useEffect(function () {
        var alive = true;
        function load() {
          // 复用与用量面板同一个接口，只取本月合计（北京时间自然月）。
          api({ action: "list" }).then(function (res) {
            if (!alive || !res || !res.records) return;
            setData(monthSummary(res.records));
          }).catch(function () {});
        }
        load();
        var stop = timer && timer.interval ? timer.interval(load, 30000) : null;
      ...
      return el("div", { className: wide ? "dsh-usage-side" : "dsh-usage-side rail" },
        el("button", {
          ...
          // 仅作展示：不猜测宿主的跳转路由。悬停显示完整明细。
          title: t("本月已消耗") + (data.cost != null ? " " + fmtMoney(data.cost) + t(" · ") + fmtInt(data.calls) + t(" 次") : ""),
          "aria-label": t("本月已消耗") + (data.cost != null ? " " + fmtMoney(data.cost) : "")
        },
          sidebarIcon(wide ? 16 : 18),
          wide
            ? el(React.Fragment, null,
                el("span", { className: "dsh-usage-sideLabel" }, t("本月已消耗")),
                data.cost != null ? el("span", { className: "dsh-usage-sideAmount" }, fmtMoney(data.cost)) : null
              )
            : null
        )
      );
    ...
    /** 从 records 汇总本月（北京时间自然月）消耗与调用次数。 */
    function monthSummary(records) {
      var nowB = new Date(Date.now() + 8 * 3600 * 1000);
      var prefix = nowB.getUTCFullYear() + "-" + pad2(nowB.getUTCMonth() + 1);
      var cost = 0, calls = 0;
      for (var i = 0; i < records.length; i++) {
        var r = records[i];
        if (!r) continue;
        if (bjKey(r.time).indexOf(prefix) !== 0) continue;
        cost += costOf(r, "auto");
        calls += 1;
      }
      return { cost: cost, calls: calls };
    }
    ...
    function apply(ctx) {
      ...
      slots.inject("sidebar.footer.action", function () {
        return slots.register(
          { name: "sidebar.footer.action", id: "usage-cost-side", order: 10, label: function () { return t("用量"); } },
          function (props) { return el(SidebarUsageEntry, { wide: props && props.wide, timer: timer }); }
        );
      });
    }

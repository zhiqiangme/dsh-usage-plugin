window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 卡片与区块的统一间距（px）：横竖一致。
    // 此前横向 10、纵向 14/24/26 各不相同，视觉上"横竖间隙不一样"；
    // 现在全部由这一个值控制，需要整体调宽/调窄改这里即可。
    var GAP = 16;

    var st = {
      root: { display: "flex", flexDirection: "column", gap: GAP, padding: "4px 0", width: "100%", maxWidth: "100%", minWidth: 0, boxSizing: "border-box" },
      ...
      // 卡片网格：列间隙 10 保持横向紧凑，行间隙 18 让竖向更透气
      // （grid 的 row-gap / column-gap 分开设，避免横竖看起来不一致）。
      cards: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(170px,1fr))", gap: GAP },
      ...
      // 3 大主卡
      // 主指标卡同理：竖版间隙加宽。
      bigCards: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: GAP },
      ...
      // 图表区
      chartGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))", gap: GAP },
      ...
      // token 明细条：同样采用较宽的竖向间隙。
      // marginTop 去掉：纵向间距统一由 st.root 的 gap 决定，避免与 gap 叠加出不一致的间距。
      tokenBar: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: GAP },
...
var api = function (payload) {
    ...
    function PriceView(props) {
      ...
      return el("div", null,
        ...
        el("div", { style: st.sec }, t("DeepSeek 官方 API 价格表")),
        el("div", { style: st.note, marginTop: 6 },
        ...
        el("div", { style: st.sec }, t("第三方平台价格与覆盖状态（2026-08-19 核验）")),

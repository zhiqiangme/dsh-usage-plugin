...
var api = function (payload) {
    ...
    // 标记类：挂在会话根容器上，用来隐藏 harness 的正文宽度调整线。
    var PANEL_MARKER_CLASS = "dsh-usage-panel-active";

    /**
     * 隐藏对话正文的左右宽度调整线。
     *
     * harness 在 phase === "active" 时给每个 conversation.view 都渲染一对
     * WidthHandle（ConversationRoot 的 nodes：它们是视图容器的**兄弟**节点，
     * 不在插件容器内部），那是给对话正文调宽用的，在用量/余额这类数据面板上
     * 没有意义，还会压在内容上。
     *
     * 因此从插件容器向上找到"包含调整线的最近祖先"（实际上是会话根容器），给它
     * 加一个标记类；CSS 用属性选择器匹配 widthHandle，避免依赖打包后的哈希类名。
     * 卸载时移除标记，不影响对话/轨迹页签（那时标记已摘掉）。
     */
    var panelStyleInstalled = false;
    function installPanelStyle() {
      if (panelStyleInstalled) return;
      panelStyleInstalled = true;
      try {
        if (typeof document === "undefined" || !document.head) return;
        if (document.querySelector("style[data-usage-plugin-panel]")) return;
        var tag = document.createElement("style");
        tag.dataset.usagePluginPanel = "";
        tag.textContent = "." + PANEL_MARKER_CLASS + " [class*=widthHandle]{display:none !important}";
        document.head.appendChild(tag);
      } catch (e) {}
    }

    /** 找到包含宽度调整线的最近祖先容器并打标记；返回撤销函数。 */
    function markWidthHandleHost(node) {
      try {
        var el = node && node.parentElement;
        while (el) {
          if (el.querySelector && el.querySelector("[class*=widthHandle]")) {
            el.classList.add(PANEL_MARKER_CLASS);
            return function () { try { el.classList.remove(PANEL_MARKER_CLASS) } catch (e) {} };
          }
          el = el.parentElement;
        }
      } catch (e) {}
    ...
    function pluginView(render, getCurrentSession) {
      return function PluginView(props) {
        ...
        React.useEffect(function () {
          installPanelStyle();
          var undo = markWidthHandleHost(hostRef.current);

...
var api = function (payload) {
    ...
    // ── 视图偏好复位（BUG 修复）────────────────────────────────────────────
    // harness 的 conversation.view 选择是**按会话持久化**的：localStorage 键
    // `dsh.conversation.<sessionId>`（见 dsh-client-ui-conversation 的
    // readConversationViewPreference / restoreView），切回某会话时会恢复上次停留的
    // 页签，于是停在「用量」「余额」而不是对话界面。
    // 修法：在**离开**该会话时把它残留的本插件页签偏好改回 chat。选择"离开时清"
    // 而不是"进入时清"，是为了不和 harness 自己那套 restoreView 抢同一个键
    // （两边操作的是不同 sessionId 的键，因此无所谓订阅先后）。
    // 只动本插件注册的两个 id：「轨迹」等内置页签与用户对它们的选择不受影响。
    var OWN_VIEW_IDS = { "usage-cost-view": true, "balance-view": true };
    var CONVERSATION_STORE_KEY = "dsh.conversation";
    var CHAT_VIEW_ID = "chat";

    /**
     * 取可用的 localStorage。
     * 显式走 window 上的实例并做一次真实读写探测：某些环境（如 Node 实验性
     * localStorage、受限 iframe）会暴露一个同名但不可用的全局，直接用它会在
     * getItem 时抛错或永远返回 null，导致偏好复位静默失效。
     */
    function viewStorage() {
      try {
        var store = (typeof window !== "undefined" && window && window.localStorage) || null;
        if (!store || typeof store.getItem !== "function" || typeof store.setItem !== "function") return null;
        // 探测：能读回自己刚写的值才算真可用。
        var probeKey = "dsh.usage.viewprobe";
    ...
    /** 读某会话持久化的视图偏好；无存储或不可解析时返回 null。 */
    function readViewPreference(sessionId) {
      var store = viewStorage();
      if (!sessionId || !store) return null;
      try {
        var raw = store.getItem(CONVERSATION_STORE_KEY + "." + sessionId);
        if (raw === null) return null;
        var stored = JSON.parse(raw);
        if (!stored || typeof stored !== "object" || typeof stored.view !== "string") return null;
        return stored.view;
      } catch (e) {
        return null;
      }
    ...
    /** 把某会话残留的本插件视图偏好改回 chat；其它字段原样保留。 */
    function resetOwnViewPreference(sessionId) {
      var view = readViewPreference(sessionId);
      if (!view || !OWN_VIEW_IDS[view]) return false;
      var store = viewStorage();
      if (!store) return false;
      try {
        var key = CONVERSATION_STORE_KEY + "." + sessionId;
    ...
    /** 订阅会话切换：离开旧会话时清掉本插件写入的页签偏好。 */
    function watchSessionViewPreference(ctx) {
      var sessions = ctx.get("sessions");
      var list = sessions && sessions.list;
      if (!list || typeof list.subscribe !== "function" || typeof list.getSnapshot !== "function") return;
      var previous = list.getSnapshot().current;
      ctx.effect(function () {
        return list.subscribe(function () {
          var next = list.getSnapshot().current;
          if (next === previous) return;
          if (previous) resetOwnViewPreference(previous);
          previous = next;
        });
      }, "usage-plugin: reset own view preference on session switch");
    }
    ...
    function apply(ctx) {
      ...
      watchSessionViewPreference(ctx);

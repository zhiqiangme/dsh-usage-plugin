 ...
 *
 * assistant/attempt 从 stream 提取最后一笔 usage，与宿主保持一致。
...
export function sampleOfEvent(event) {
  ...
  // 与官方 tokenUsage 投影的 usageOf 逐条对齐（dsh-token-meter/lib/index.js）：
  //   ① assistant/message 的顶层 data.usage 优先；
  //   ② 否则回退到 stream 里最后一个 usage chunk，assistant/attempt 也走这条。
  // 只做 ① 会漏掉"有 attempt 但顶层没有 usage"的调用（重试/中断的尝试），
  // 累计值因此比宿主状态栏少一截（实测差约 1M）。
  if (event.type === 'assistant/message' || event.type === 'assistant/attempt') {
    const turn = toIndex(data.turn)
    const step = toIndex(data.step)
    if (event.type === 'assistant/message' && data.usage) {
      return { usage: data.usage, turn, step }
    }
    const usage = lastStreamUsage(data.stream)
    if (usage) return { usage, turn, step }
    return null
  }
  return null
}
...
/**
 * 取 stream 中最后一个 usage chunk 的 usage。
 * 对应官方的 lastAssistantStreamChunk(stream, 'usage')：从后往前找第一个
 * 带 usage 的 chunk（兼容 chunk 直接是 usage、或包在 chunk 字段里两种形状）。
 */
function lastStreamUsage(stream) {
  ...
  for (let i = stream.length - 1; i >= 0; i--) {
    ...
    const chunk = rec.chunk && typeof rec.chunk === 'object' ? rec.chunk : rec
 ...
 *
 * 新记录用 sessionId|slotSeq，重试和非连续步骤分别占槽。
 * 同槽位重复结算替换，增量折叠状态与游标一起持久化。
 * 拿不到 turn/step 时退化为 sessionId|seq。
 */
export function scanRecordKey(record) {
  const session = String((record && record.sessionId) || '')
  if (!session) return ''
  if (Number.isSafeInteger(record.slotSeq) && record.slotSeq >= 0) return session + '|slot' + record.slotSeq
...
export function reduceSessionEvents(events, options = {}) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    // 宿主只替换最近一次结算；重试关闭该槽位，新的用量必须独立累计。
    if (event.type === 'llm/retry-started' && state.lastSample
      && state.lastSample.turn === event.data.turn && state.lastSample.step === event.data.step) state.lastSample = null
    const sample = sampleOfEvent(event)
    if (!sample) continue
    if (!state.lastSample || sample.turn < 0 || sample.step < 0
      || state.lastSample.turn !== sample.turn || state.lastSample.step !== sample.step) {
      state.lastSample = { turn: sample.turn, step: sample.step, slotSeq: seq }
    }
    ...
    records.push({
      ...
      turn: sample.turn,
      step: sample.step,
      slotSeq: state.lastSample.slotSeq,
      ...
      // 只存上报值：用未命中兜底伪造 cacheWrite 会让四桶合计双计未命中（与探针同口径）。
      cacheWriteTokens: num(sample.usage.cacheWriteTokens),
 ...
 *   3. 深扫仅吸收身份、路由、用量、时间均匹配的唯一普通探针，保留费用元数据。
 *
...
export function mergeScannedRecords(existing, incoming, options = {}) {
  ...
  // 建立索引并清理旧版同槽位副本，避免长历史扫描退化为平方复杂度。
  const scanIndex = new Map(), legacyIndex = new Map(), probes = new Map()
  const compact = []
  for (const r of existing) {
    if (r.origin === SCAN_ORIGIN) {
      const key = scanRecordKey(r)
      if (scanIndex.has(key)) {
        compact[scanIndex.get(key)] = withBillingMetadata(r, compact[scanIndex.get(key)])
        continue
      }
      scanIndex.set(key, compact.length)
      if (r.slotSeq === undefined) legacyIndex.set(JSON.stringify([r.sessionId, r.seq]), compact.length)
    } else {
      const bucket = probes.get(r.sessionId) || []
      bucket.push(compact.length)
      probes.set(r.sessionId, bucket)
    }
    compact.push(r)
  }
  existing.length = 0
  for (const r of compact) existing.push(r)
  // 先折叠本批重复结算，避免相同槽位消耗多个探针。
  const folded = new Map()
  for (const r of incoming || []) if (r) folded.set(scanRecordKey(r), r)
  for (const record of folded.values()) {
    const key = scanRecordKey(record)
    const at = scanIndex.get(key) ?? legacyIndex.get(JSON.stringify([record.sessionId, record.seq]))
    if (at !== undefined) {
      const oldKey = scanRecordKey(existing[at])
      scanIndex.delete(oldKey)
      existing[at] = withBillingMetadata(record, existing[at])
      scanIndex.set(key, at)
      stats.replaced++
      continue
    }
    const matches = options.deep === true
      ? (probes.get(record.sessionId) || []).filter((i) => existing[i].origin !== SCAN_ORIGIN && sameCall(existing[i], record)) : []
    // 匹配不唯一时保留原始记录，不贸然覆盖其他请求。
    if (matches.length === 1) {
      existing[matches[0]] = withBillingMetadata(record, existing[matches[0]])
      scanIndex.set(key, matches[0])
      stats.absorbed++
    } else if (matches.length > 1) {
      // 无法区分多个同值请求时保留探针，不再追加一笔导致双计。
      stats.ambiguous = (stats.ambiguous || 0) + 1
    } else {
      scanIndex.set(key, existing.length)
...
/** 只有会话、路由、用量及请求时间都匹配的普通调用才能吸收，辅助调用始终独立。 */
export function sameCall(probe, scanned) {
  if (!probe || probe.purpose) return false
  if (!probe.sessionId || probe.sessionId !== scanned.sessionId) return false
  if (probe.model !== scanned.model || probe.provider !== scanned.provider) return false
  if (!['inputTokens', 'outputTokens', 'cacheReadTokens', 'cacheWriteTokens', 'reasoningTokens']
    .every((k) => Number(probe[k] || 0) === Number(scanned[k] || 0))) return false
  const a = Number(probe.time || 0), b = Number(scanned.time || 0)
  return a > 0 && b >= a && b - a <= OVERLAP_WINDOW_MS
}

/** 用量和结算时间以日志为准，费用沿用原请求时间与已保存汇率。 */
export function withBillingMetadata(record, source) {
  return { ...record, purpose: source.purpose || record.purpose,
    finishReason: source.finishReason || record.finishReason,
    interrupted: !!source.interrupted,
    ...(source.usdCnyRate > 0 ? { usdCnyRate: source.usdCnyRate, fxDate: source.fxDate || '' } : {}),
    pricingTime: source.pricingTime || source.time }
}

/** 面板按精确事件身份或唯一探针匹配恢复费用元数据，不能拿当前汇率冒充历史汇率。 */
export function enrichBillingMetadata(scanned, existing) {
  const available = [...existing]
  return scanned.map((r) => {
    let candidates = available.filter((p) => p.origin === SCAN_ORIGIN && p.sessionId === r.sessionId && p.seq === r.seq)
    if (!candidates.length) candidates = available.filter((p) => sameCall(p, r))
    if (candidates.length !== 1) return r
    const p = candidates[0]
    available.splice(available.indexOf(p), 1)
    return withBillingMetadata(r, p)
  })
}
 ...
 * 可供精确对齐）。部分覆盖的会话需要用深扫补齐，不能认为该集合证明日志完整。
 */
  ...
  for (const [id, entry] of Object.entries(sessions)) {
    ...
    cache.sessions[id] = {
      ...
      lastRoute: entry.lastRoute && typeof entry.lastRoute === 'object'
        ...
        : null,
      lastSample: entry.lastSample && [entry.lastSample.turn, entry.lastSample.step, entry.lastSample.slotSeq].every(Number.isSafeInteger)
 ...
 *
 * 重扫按结算槽位替换，增量读取携带 lastSample；深扫从头重建该状态。
 *
...
export async function scanSession(persistence, id, entry, options = {}) {
  ...
  try {
    ...
    const firstScan = options.deep === true || !Number.isSafeInteger(entry.seq) || entry.seq < 0
    ...
    const state = firstScan || !contiguous ? { lastRoute: null }
      : { lastRoute: entry.lastRoute || null, lastSample: entry.lastSample || null }
    const result = reduceSessionEvents(events, { sessionId: id, state })
    entry.lastSample = state.lastSample || null
...
export async function scanSessions(persistence, options = {}) {
  ...
  for (const snapshot of sessions) {
    ...
    if (options.deep !== true && entry.revision !== '' && snapshot.revision !== '' && snapshot.revision === entry.revision) {
      stats.skipped++
      continue
    }
